...
class Replace s => IsRegex re s where
  ...
  makeSearchReplace     :: (Functor m,Monad m,IsRegex re s, MonadFail m) => s -> s -> m (SearchReplace re s)
  -- | compiling a 'SearchReplace' template specifing the 'SimpleREOptions' for the RE
  makeSearchReplaceWith :: (Functor m,Monad m,IsRegex re s, MonadFail m) => SimpleREOptions -> s -> s -> m (SearchReplace re s)
